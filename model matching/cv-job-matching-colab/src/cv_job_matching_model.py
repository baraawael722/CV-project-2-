...
class CVJobMatcher:
    ...
    def create_training_data(self, cvs_df, jobs_df, sample_size=10000):
        ...
        for idx, cv_row in cvs_df.iterrows():
            ...
            matching_jobs = jobs_df[jobs_df['Job Title'].str.contains(category, case=False, na=False)]
        ...
        for idx, cv_row in cvs_df.iterrows():
            ...
            non_matching_jobs = jobs_df[~jobs_df['Job Title'].str.contains(category, case=False, na=False)]
    ...
    def train(self, cvs_file, jobs_file, epochs=50, batch_size=32, learning_rate=0.001):
        ...
        for epoch in range(epochs):
            ...
            for batch in train_loader:
                ...
                # Gradient Clipping
                torch.nn.utils.clip_grad_norm_(self.matching_model.parameters(), max_norm=1.0)
            ...
            if val_acc > best_val_acc:
                ...
                patience_counter = 0
                # حفظ أفضل نموذج
                torch.save(self.matching_model.state_dict(), 'best_matching_model.pth')
                print(f"✅ تم حفظ أفضل نموذج! Validation Accuracy: {val_acc:.2f}%\n")
            else:
                patience_counter += 1
        ...
        # تحميل أفضل نموذج
        self.matching_model.load_state_dict(torch.load('best_matching_model.pth'))
    ...
    def find_top_matches(self, cv_text, job_descriptions, top_k=10):
        ...
        with torch.no_grad():
            ...
            for idx, job_emb in enumerate(job_embeddings):
                job_tensor = torch.FloatTensor(job_emb).unsqueeze(0).to(self.device)
    ...
    def load_model(self, path='cv_job_matcher.pkl'):
        ...
        self.matching_model = SiameseMatchingNetwork(embedding_dim=self.embedding_dim).to(self.device)
...
def main():
    ...
    best_accuracy = matcher.train(
        cvs_file='data/dataa.csv',
        jobs_file='data/jobs_clean.csv',
        epochs=50,
        batch_size=32,
        learning_rate=0.001
    )
    ...
    # حفظ النموذج
    matcher.save_model('models/cv_job_matcher_final.pkl')
