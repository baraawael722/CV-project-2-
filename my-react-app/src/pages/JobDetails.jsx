  ...
  useEffect(() => {
    const fetchJobDetails = async () => {
      try {
        ...
        const analysisRes = await fetch(`http://localhost:5000/api/ml/analyze-job/${jobId}`, {
          headers: { Authorization: `Bearer ${token}` }
        })
            ...
            <div className="bg-white rounded-xl shadow-lg p-8 border border-gray-200">
              <div className="flex items-start gap-6 mb-6">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xl text-gray-600 font-medium mb-4">{job.company || 'Company Name'}</p>
            ...
            <div className="bg-white rounded-xl shadow-lg p-8 border border-gray-200">
              ...
              <div className="flex flex-wrap gap-3">
                {(job.requiredSkills || job.skills || []).map((skill, index) => (
                  <span key={index} className="px-4 py-2 bg-indigo-50 text-indigo-700 border-2 border-indigo-200 rounded-lg text-sm font-semibold">
                    {skill}
                  </span>
                ))}
              </div>
            ...
            <div className="bg-white rounded-xl shadow-lg p-8 border border-gray-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <p className="text-lg text-gray-900">{job.department || 'N/A'}</p>
                </div>
                <div>
                  <h3 className="text-sm font-semibold text-gray-500 uppercase mb-2">Experience Level</h3>
                  <p className="text-lg text-gray-900">{job.experienceLevel || 'N/A'}</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="sticky top-8 space-y-6">
                ...
                {analysisLoading ? (
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="text-center p-6 bg-white rounded-lg shadow-sm">
                      <p className="text-sm font-semibold text-gray-500 uppercase mb-2">Match Score</p>
                      <div className={`text-5xl font-bold mb-2 ${
                        skillAnalysis.matchScore >= 80 ? 'text-green-600' :
                        skillAnalysis.matchScore >= 60 ? 'text-blue-600' :
                        skillAnalysis.matchScore >= 40 ? 'text-yellow-600' :
                        'text-red-600'
                      }`}>
                      ...
                      <div className="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
                        <div 
                          className={`h-full rounded-full transition-all ${
                            skillAnalysis.matchScore >= 80 ? 'bg-green-600' :
                            skillAnalysis.matchScore >= 60 ? 'bg-blue-600' :
                            skillAnalysis.matchScore >= 40 ? 'bg-yellow-600' :
                            'bg-red-600'
                          }`}
                        ...
                        <div className="space-y-2">
                          {skillAnalysis.matchedSkills.map((skill, index) => (
                            <div key={index} className="flex items-center gap-2 p-2 bg-green-50 rounded-md">
                              ...
                              <span className="text-sm font-medium text-green-800">{skill.skill || skill}</span>
                              {skill.confidence && (
                                <span className="ml-auto text-xs text-green-600 font-semibold">{skill.confidence}</span>
                              )}
                            </div>
                          ))}
                        </div>
                          ...
                          {skillAnalysis.missingSkills.map((skill, index) => (
                            <div key={index} className="p-3 bg-red-50 rounded-md border border-red-200">
                              <div className="flex items-start gap-2 mb-2">
                                ...
                                <div className="flex-1">
                                  <span className="text-sm font-medium text-red-800 block">{skill.skill || skill}</span>
                                  {skill.confidence && (
                                    <span className="text-xs text-red-600 font-semibold">{skill.confidence}</span>
                                  )}
                                </div>
                              </div>
                  ...
                  <div className="text-center py-8 bg-white rounded-lg">
                    ...
                    <p className="text-sm text-gray-500">Upload your CV to get personalized insights</p>
                  </div>
                )}
              </div>
              ...
              <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
                ...
                <div className="space-y-3 text-gray-700">
                  <p className="font-semibold text-lg">{job.company || 'Company Name'}</p>
                  <p className="text-sm leading-relaxed">
                    {job.companyDescription || 'Join a dynamic team working on innovative projects. We offer competitive benefits and a great work environment.'}
                  </p>
                </div>
              </div>
            </div>
